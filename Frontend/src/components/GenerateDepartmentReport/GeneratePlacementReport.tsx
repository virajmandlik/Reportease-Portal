  ...
  const handleGenerateReport = async (format: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsReportAccessDialogOpen(true); // Open the password dialog
  
        // Optionally, download the password CSV
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6">
        ...
        <div className="flex flex-col items-center justify-center">
          <form onSubmit={handleSubmit} className="w-full max-w-md">
            ...
            {isCustomized && <PlacementDataTableDemo setSelectedOptions={setSelectedOptions} />}
